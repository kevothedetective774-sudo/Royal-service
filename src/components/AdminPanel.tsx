...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [newPkgForm, setNewPkgForm] = useState<Omit<InvestmentPackage, 'id'>>({
    ...
    runningSchedule: 'all',
    customRunningDays: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'],
            ...
            {packages.map((pkg) => (
              ...
              >
                <div>
                  ...
                  {/* Running Schedule & Visibility Indicator */}
                  <div className="flex items-center justify-between text-[11px] bg-[#111726] p-2.5 rounded-xl border border-slate-800/80 mt-2.5">
                    <div className="flex items-center gap-1.5 text-purple-300">
                      <Calendar className="w-3.5 h-3.5 text-purple-400" />
                      <span className="font-semibold">
                        {pkg.runningSchedule === 'weekdays' 
                          ? 'Weekdays (Mon–Fri)' 
                          : pkg.runningSchedule === 'weekends' 
                          ? 'Weekends (Sat–Sun)' 
                          : pkg.runningSchedule === 'custom'
                          ? (pkg.customRunningDays?.join(', ') || 'Custom Days')
                          : 'Mon – Sun (All 7 Days)'}
                      </span>
                    </div>
                    <div>
                      {pkg.showRunningDaysToUsers !== false ? (
                        <span className="flex items-center gap-1 text-emerald-400 font-medium text-[10px] bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">
                          <Eye className="w-3 h-3" /> Visible to users
                        </span>
                      ) : (
                        <span className="flex items-center gap-1 text-slate-400 font-medium text-[10px] bg-slate-800 px-2 py-0.5 rounded-full border border-slate-700">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSavePackageEdit} className="space-y-3.5 text-xs">
                ...
                {/* Plan Running Days Schedule */}
                <div className="p-3 bg-[#0a0e17] rounded-xl border border-slate-800 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <label className="font-semibold text-slate-300 text-xs flex items-center gap-1.5">
                      <Clock className="w-3.5 h-3.5 text-purple-400" />
                      <span>Plan Running Days (ROI Cycle)</span>
                    </label>
                    <span className="text-[10px] text-slate-400">Controls active yield days</span>
                  </div>

                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <button
                      type="button"
                      onClick={() => setEditingPkg({ ...editingPkg, runningSchedule: 'all' })}
                      className={`py-1.5 px-2 rounded-lg border text-center transition cursor-pointer ${
                        (editingPkg.runningSchedule || 'all') === 'all'
                    ...
                    <button
                      type="button"
                      onClick={() => setEditingPkg({ ...editingPkg, runningSchedule: 'weekdays' })}
                      className={`py-1.5 px-2 rounded-lg border text-center transition cursor-pointer ${
                        editingPkg.runningSchedule === 'weekdays'
                          ? 'bg-purple-600/20 border-purple-500 text-purple-200 font-bold'
                          : 'border-slate-800 text-slate-400 hover:text-slate-200 hover:border-slate-700'
                      }`}
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => setEditingPkg({ ...editingPkg, runningSchedule: 'weekends' })}
                      className={`py-1.5 px-2 rounded-lg border text-center transition cursor-pointer ${
                        editingPkg.runningSchedule === 'weekends'
                          ? 'bg-purple-600/20 border-purple-500 text-purple-200 font-bold'
                          : 'border-slate-800 text-slate-400 hover:text-slate-200 hover:border-slate-700'
                      }`}
                    >
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-800/80">
                    <div>
                      <span className="text-slate-300 font-medium block">Show Running Days to Users</span>
                      <span className="text-[10px] text-slate-500">Display running schedule tag on the public package card</span>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={editingPkg.showRunningDaysToUsers !== false}
                        onChange={(e) => setEditingPkg({ ...editingPkg, showRunningDaysToUsers: e.target.checked })}
              ...
              <form onSubmit={handleCreatePackageSubmit} className="space-y-3 text-xs">
                ...
                <div className="p-3 bg-[#0a0e17] rounded-xl border border-slate-800 space-y-2.5">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <button
                      type="button"
                      onClick={() => setNewPkgForm({ ...newPkgForm, runningSchedule: 'all' })}
                      className={`py-1.5 px-2 rounded-lg border text-center transition cursor-pointer ${
                        (newPkgForm.runningSchedule || 'all') === 'all'
                          ? 'bg-purple-600/20 border-purple-500 text-purple-200 font-bold'
                          : 'border-slate-800 text-slate-400 hover:text-slate-200 hover:border-slate-700'
                      }`}
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => setNewPkgForm({ ...newPkgForm, runningSchedule: 'weekdays' })}
                      className={`py-1.5 px-2 rounded-lg border text-center transition cursor-pointer ${
                        newPkgForm.runningSchedule === 'weekdays'
                          ? 'bg-purple-600/20 border-purple-500 text-purple-200 font-bold'
                          : 'border-slate-800 text-slate-400 hover:text-slate-200 hover:border-slate-700'
                      }`}
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => setNewPkgForm({ ...newPkgForm, runningSchedule: 'weekends' })}
                      className={`py-1.5 px-2 rounded-lg border text-center transition cursor-pointer ${
                        newPkgForm.runningSchedule === 'weekends'
                          ? 'bg-purple-600/20 border-purple-500 text-purple-200 font-bold'
                          : 'border-slate-800 text-slate-400 hover:text-slate-200 hover:border-slate-700'
                      }`}
                    >
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-800/80">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={newPkgForm.showRunningDaysToUsers !== false}
                        onChange={(e) => setNewPkgForm({ ...newPkgForm, showRunningDaysToUsers: e.target.checked })}
                        className="sr-only peer"
                      />
                      <div className="w-9 h-5 bg-slate-800 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-purple-600"></div>
                    </label>
